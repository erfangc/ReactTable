...
var Row = React.createClass({displayName: 'Row',
    render: function () {
        ...
        var cells = [];
        var firstCell;
        var firstColumn = this.props.columnDefs[0];
        if (this.props.data.isDetail) {
            firstCell = React.DOM.td({style: tdStyle, key: this.props.data[firstColumn]}, this.props.data[firstColumn]);
        } else {
            firstCell =
                (
                    React.DOM.td({style: tdStyle, key: this.props.data[firstColumn]}, 
                        React.DOM.a({onClick: this.props.toggleHide.bind(null, this.props.data), className: "btn-link"}, 
                            React.DOM.strong(null, this.props.data[firstColumn])
                        )
                    )
                );
        }
