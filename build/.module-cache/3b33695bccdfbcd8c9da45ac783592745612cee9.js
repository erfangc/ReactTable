...
/* Utility Functions */
function isSubSectorOf(subSectorCandidate, superSect) {
    // lower length in SP means higher up on the chain
    if (subSectorCandidate.length <= superSect.length)
        return false;
    for (var i = 0; i < superSect.length; i++) {
        if (subSectorCandidate[i] != superSect[i])
            return false;
    }
    return true;
}
