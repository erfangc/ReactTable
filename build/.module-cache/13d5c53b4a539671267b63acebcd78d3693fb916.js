...
/* Utility Functions */
function isSubSectorOf(subSectorCandidate, superSectorCnadi) {
    // lower length in SP means higher up on the chain
    if (subSectorCandidate.length <= superSectorCnadi.length)
        return false;
    for (var i = 0; i < superSectorCnadi.length; i++) {
        if (subSectorCandidate[i] != superSectorCnadi[i])
            return false;
    }
    return true;
}
